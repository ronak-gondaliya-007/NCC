...
import { db_function, Schema } from '../interface/database.interface';
...
async function bulkWrite({ collection, documents }: db_function) {
  const result = await (schema as Schema)[collection].bulkWrite(documents);
  return result;
}
...
async function paginate({ collection, query, options }: db_function) {
  const result = await (schema as any)[collection].paginate(query, options);
  return result;
}
